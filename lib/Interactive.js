...
class Interactive {
  constructor (layer, collisionFn, cursorStyle, cursorFn) {
    ...
    this._bindings.push(this._layer.surface.on('down', this._down, collisionFn));
    this._bindings.push(this._layer.surface.on('up', this._up, collisionFn));
    this._bindings.push(this._layer.surface.on('tap', this._tap, collisionFn));
    this._bindings.push(this._layer.surface.on('over', this._over, collisionFn));
    this._bindings.push(this._layer.surface.on('out', this._out, collisionFn));

    if (cursorStyle || cursorFn) {
      cursorFn = cursorFn || collisionFn;
      this._bindings.push(this._layer.surface.cursor('pointer', cursorFn));
    }
  }
