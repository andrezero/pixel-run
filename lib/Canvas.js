...
class Canvas {
  constructor (container, config) {
    ...
    element.classList.add(CANVAS_CLASS_NAME);
  ...
  scaleText (val, min = 0) {
    return Math.max(Math.round(val * this.scale), min / this.transform);
  }
  ...
  scalePoint (point) {
    return {
      x: Math.round(this.scale * point.x),
      y: Math.round(this.scale * point.y)
    };
  }
