...
class Canvas {
  constructor (container, config) {
    ...
    scaler.appendChild(this._surface._element);
  ...
  bindEvent (eventName, callback, rect) {
    return this._surface.bindEvent(eventName, callback, rect);
  }
  ...
  scaleSize (size) {
    return {
      w: Math.round(this.scale * (size.x + 1)),
      h: Math.round(this.scale * (size.y + 1))
    };
  }
